...
Not to be confused with the swagger framework versioning, the swagger specification is versioned for compatibility.  You can see information about previous versions in the [wiki](wiki/Changelog.md).
...
* An `authorizations` section was added to define what authorization schemes were supported for the API.  See the [authorizations](wiki/authorizations.md) section for details on this section.  It is optional and not required for 1.2 complicane.

* An `info` section was added to define metadata about the API.  It too is optional but highly recommended--the metadata is also presented directly in the [swagger-ui](https://github.com/wordnik/swagger-ui) console for easy access.  See the [API Info](wiki/api-info.md) section for details.
...
The `path` and `resourcePath` attributes were inconsistent with leading slashes, causing extra validations.
